...
public class ThreadManager {
    ...
    public static class ThreadPool{
        ...
        public  void execute(Runnable r){
            if(executor == null){
                //线程池执行者。
                 //参1:核心线程数： 一般核心线程不受存活时间的影响（除非你设置），当前线程数小于核心线程数时，
                // 他会继续创建线程，不管有没有空闲线程
                // 参2:最大线程数;  当设置的线程 队列满了，当前线程数小于最大线程时，他会继续创建线程，
                // 如果使用了无界(没有大小 LinkedBlockingDeque)的任务队列，这个参数就没意义了
                ...
                // 参5:线程队列;    用于保存等待执行的任务的阻塞队列
                                 ...
                                 //SynchronousQueue：一个不存储元素的阻塞队列。每个插入操作必须等上一个元素被移除之后，
                                      // 否则插入操作一直处于阻塞状态，吞吐量通常要高于LinkedBlockingQueue，静态工厂方法Executors.newCachedThreadPool使用了这个队列。
